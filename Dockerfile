FROM jlesage/baseimage-gui:debian-13-v4

ENV DEBIAN_FRONTEND=noninteractive \
    DISPLAY=:1 \
    SCREEN_WIDTH=1280 \
    SCREEN_HEIGHT=720 \
    SCREEN_DEPTH=24 \
    APP_NAME="Wireshark" \
    WIRESHARK_RUN_DUMPCAP_AS_ROOT=1

# ----------------------------
# UI / noVNC tweak
# ----------------------------
RUN sed -i "s/UI.initSetting('resize', resize);/UI.initSetting('resize', 'remote');/g" /opt/noVNC/app/ui.js

# ----------------------------
# Pre-seed Wireshark config (avoid prompts)
# ----------------------------
RUN echo "wireshark-common wireshark-common/install-setuid boolean false" | debconf-set-selections

# ----------------------------
# Core system + Wireshark (FULL GUI)
# + fix dpkg safety for CI builds
# ----------------------------
RUN apt-get update && apt-get install -y --no-install-recommends \
        wireshark \
        curl \
        jq \
        ca-certificates \
        git && \
    dpkg --configure -a || true && \
    apt-get -f install -y && \
    apt-get clean && \
    rm -rf /var/lib/apt/lists/*

# ----------------------------
# Install cshargextcap (Packet capture extcap)
# ----------------------------
RUN set -eux; \
    ARCH=$(dpkg --print-architecture); \
    LATEST_RELEASE=$(curl -fsSL https://api.github.com/repos/siemens/cshargextcap/releases/latest | jq -r .tag_name); \
    test -n "$LATEST_RELEASE"; \
    curl -fL -o /tmp/cshargextcap.deb \
      "https://github.com/siemens/cshargextcap/releases/download/${LATEST_RELEASE}/cshargextcap_${LATEST_RELEASE#v}_linux_${ARCH}.deb"; \
    dpkg -i /tmp/cshargextcap.deb || apt-get -f install -y; \
    rm -f /tmp/cshargextcap.deb

# ----------------------------
# Install AnySec dissectors
# ----------------------------
RUN set -eux; \
    git clone https://github.com/xavixava/anysec-dissectors.git /tmp/anysec-dissectors; \
    mkdir -p /usr/lib/x86_64-linux-gnu/wireshark/plugins; \
    cp -r /tmp/anysec-dissectors/4.4_anysec_plugins \
        /usr/lib/x86_64-linux-gnu/wireshark/plugins/; \
    chmod -R 755 /usr/lib/x86_64-linux-gnu/wireshark/plugins/4.4_anysec_plugins; \
    rm -rf /tmp/anysec-dissectors

# ----------------------------
# Start script
# ----------------------------
COPY startapp.sh /startapp.sh
RUN chmod +x /startapp.sh